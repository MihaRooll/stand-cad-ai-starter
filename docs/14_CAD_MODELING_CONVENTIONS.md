...
**Residual uncapped penetrating classes (P2):** none beyond the capped classes in the table above (D-099 pruned dead INTERLOCK/MAINS patterns). Do not apply a single global ~40k ceiling.

## 11. Kinematic-group mates — no blanket exemption (D-086)

**Do not** treat shared `LOWER_KINEMATIC_GROUP` / `UPPER_KINEMATIC_GROUP` membership as an unconditional `is_mating` pass — that greenwashed deep burial (e.g. synthetic TRAY↔EQUIP). Intentional same-group contacts must be on **`MATING_PAIRS`**, **`is_door_mate`**, **`is_open_front_kinematic_contact`**, or a measured skin ceiling.
...
**Cross-tier staggered Y-overlap (`is_staggered_tier_y_overlap`, D-097):** tiers share front-face Y (D-033) and stack in Z — cross-tier Y overlap is intentional, not collision. Applies to lower×upper marker prefixes (plotter/tray/slide/rail/softstop/vib per tier). Live max **0 mm³** across all cross-tier marker pairs with `y_overlap > threshold` (2026-08-08 transport / service_p1 / service_p2). Exempt when `y_overlap > threshold` **and** `intersection_volume <= STAGGERED_TIER_MAX_BEARING_MM3 + threshold` (**500 mm³** ceiling). Pre-fix burial `EQUIP-PLOTTER1-001` ↔ `FRAME-RAIL-TRAY-UPPER-*` (~**43875 mm³**) must not silent-green via Y-overlap alone. Deep burial must fail.

**Cross-tier TRAY/SLIDE↔rail share_face exclude (D-098):** uncapped `TRAY-`/`SLIDE-` ↔ `FRAME-RAIL-TRAY` `_share_face_if_prefix` in `is_mating()` does **not** apply when the pair is cross-tier (`TRAY-LOWER-`/`SLIDE-LOWER-` ↔ `FRAME-RAIL-TRAY-UPPER-` or upper tray/slide ↔ lower rail). Those pairs defer to `is_staggered_tier_y_overlap` (D-097 **500 mm³** ceiling). Same-tier share_face and same-tier `MATING_PAIRS` slide↔rail paths unchanged. Pre-fix bypass: buried opposite-tier rail with `aabb_share_face` True and `inter_vol` ≫ 500 (e.g. `TRAY-LOWER-001` ↔ `FRAME-RAIL-TRAY-UPPER-*` ~**1222650 mm³**) returned `is_mating` True via share_face before staggered check. Live cross-tier TRAY/SLIDE↔opposite rail share_face max **0 mm³** (transport / service_p1 / service_p2).
...
**Service cover ↔ base rail (`is_cover_svc_frame_base_bearing`, D-096):** `COVER-SVC-001` ↔ `FRAME-RAIL-BASE-REAR-001` mates via **`COVER-SVC-`/`FRAME-RAIL-BASE` `_share_face_if_prefix`** only. Live max **7350.0 mm³** (2026-08-08 transport). Exempt when `intersection_volume <= COVER_SVC_FRAME_BASE_MAX_BEARING_MM3 + threshold` (**10000 mm³** ceiling). Coplanar share_face burial cannot bypass. Deep burial must fail.

**Service cover ↔ rear corner post (`is_cover_svc_frame_post_bearing`, D-096):** `COVER-SVC-001` ↔ `FRAME-POST-RL/RR-001` on `PENETRATING_JOINT_PATTERNS` and via **`COVER-SVC-`/`FRAME-POST-R` `_share_face_if_prefix`**. Live max **122.9474 mm³** (2026-08-08 transport). Exempt when `intersection_volume <= COVER_SVC_FRAME_POST_MAX_BEARING_MM3 + threshold` (**500 mm³** ceiling). Both penetrating and share_face paths volume-gated. Deep burial must fail.

**Residual uncapped `MATING_PAIRS` / share_face (P2):** SOFT↔TRAY, shelf/org, media — D-087 residual P2 where applicable (D-099 pruned dead INTERLOCK/MAINS/EDGEGUARD/REARSUPPORT/AIRPATH/SVC-INSERT allowlists).

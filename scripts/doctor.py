...
def check_concept_artifacts() -> CheckResult:
    ...
    manifests = list(CONCEPT_DIR.glob("*.manifest.json"))
    glbs = list(CONCEPT_DIR.glob("*.glb"))
    steps = list(CONCEPT_DIR.glob("*.step"))
    if not manifests or not glbs:
        return CheckResult(
            "concept artifacts",
            False,
            f"Found {len(manifests)} manifest(s), {len(glbs)} GLB(s), {len(steps)} STEP(s)",
            "Run `uv run python scripts/regenerate.py` to export concept mesh bundles.",
        )
    return CheckResult(
        "concept artifacts",
        True,
        f"{len(manifests)} manifest(s), {len(glbs)} GLB(s), {len(steps)} STEP(s) "
        "under output/concept/",
    )
